...
The full loop has also run live against the Binance spot testnet: a question through the owner API, a proposal to buy $15.00 of ETH at market, the rulebook's verdict (allowed, approval required), the owner's approval, and testnet order `9113772` filled for 0.0059 ETH at 2501.29. Those lines are the captured ledger sequence the landing page plays back, exported from the real ledger into `packages/web/public/ledger-sample.json`.
   ```
