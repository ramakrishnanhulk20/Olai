...
[ol.] docs/security/threat-model.md:169: `OWNER_TOKEN_PREFIX = 'ol.'`, `assertOwnerToken` (checked at boot, refuses to start
[ol.] docs/security/threat-model.md:251: the fact that session ids (`ol-<uuid>`) and the owner token (`ol.` prefix,
...
[ol.] packages/web/content/docs/security/threat-model.md:141: **The owner token prefix and constant-time compare.** `src/api/auth.ts`: the `ol.` prefix, a
...
[ol.] README.md:276: Copy `.env.example` to `.env` and fill in two keys: `ANTHROPIC_API_KEY` from console.anthropic.com, and a free Binance spot testnet key from `https://testnet.bi
[wallet address] README.md:323: Run on 2026-09-06 with `OLAI_DRY_RUN=false OLAI_PROVE_SPEND=yes`. The receipt for that settlement, read from a BNB Smart Chain node: status success, block 12027
[ol.] README.md:334: | `OLAI_OWNER_TOKEN` | The password to Olai's own API. Must start with `ol.` and be at least 24 characters. Anyone holding it can approve trades. |
```
...
```
history is 100402 lines long

[ol.] git log -p --all, line 300: Set OLAI_OWNER_TOKEN in .env, it starts with ol.
[ol.] git log -p --all, line 2382: +Copy `.env.example` to `.env` and fill in two keys: `ANTHROPIC_API_KEY` from console.anthropic.com, and a free Binance spot testnet key from `https://testnet.b
[wallet address] git log -p --all, line 2436: -Run on 2026-09-06 with `OLAI_DRY_RUN=false OLAI_PROVE_SPEND=yes`. The receipt for that settlement, read from a BNB Smart Chain node: status success, block 1202
[wallet address] git log -p --all, line 2437: +Run on 2026-09-06 with `OLAI_DRY_RUN=false OLAI_PROVE_SPEND=yes`. The receipt for that settlement, read from a BNB Smart Chain node: status success, block 1202
[ol.] git log -p --all, line 2511: +const OWNER_TOKEN = 'ol.desk-harness-token-not-for-production-1';
[PRIVATE_KEY] git log -p --all, line 5519: @@ -49,7 +49,9 @@ const KEY_ASSIGNMENT = /(PRIVATE_KEY|MNEMONIC)\s*[:=]\s*["']?[A-Za-z0-9+/=_-]{16
[MNEMONIC] git log -p --all, line 5519: @@ -49,7 +49,9 @@ const KEY_ASSIGNMENT = /(PRIVATE_KEY|MNEMONIC)\s*[:=]\s*["']?[A-Za-z0-9+/=_-]{16
[wallet address] git log -p --all, line 36454: Run on 2026-09-06 with `OLAI_DRY_RUN=false OLAI_PROVE_SPEND=yes`. The receipt for that settlement, read from a BNB Smart Chain node: status success, block 12027
[ol.] git log -p --all, line 36716: +# "ol." and make it at least 24 characters. Anyone holding it can approve trades.
[ol.] git log -p --all, line 36717: +OLAI_OWNER_TOKEN=ol.
[wallet address] git log -p --all, line 37399: +Run on 2026-09-06 with `OLAI_DRY_RUN=false OLAI_PROVE_SPEND=yes`. The receipt for that settlement, read from a BNB Smart Chain node: status success, block 1202
[ol.] git log -p --all, line 37411: +| `OLAI_OWNER_TOKEN` | The password to Olai's own API. Must start with `ol.` and be at least 24 characters. Anyone holding it can approve trades. |
[ol.] git log -p --all, line 37715: +  `OWNER_TOKEN_PREFIX = 'ol.'`, `assertOwnerToken` (checked at boot, refuses to start
[ol.] git log -p --all, line 37792: +  the fact that session ids (`ol-<uuid>`) and the owner token (`ol.` prefix,
[ol.] git log -p --all, line 38185: +- The token lives in its own namespace: it starts with `ol.` and is at least 24 characters, while
[ol.] git log -p --all, line 39514: +| `OLAI_OWNER_TOKEN` | A secret you invent. It has to start with `ol.` and be at least 24 characters. Anyone holding it can approve trades. |
[ol.] git log -p --all, line 40615: +- One user, one bearer token, its own `ol.` namespace, compared in constant time, checked at boot.
[ol.] git log -p --all, line 40782: +**The owner token prefix and constant-time compare.** `src/api/auth.ts`: the `ol.` prefix, a
[ol.] git log -p --all, line 73047: +    const guessedPrefix = await call(harness.base, '/api/ledger', { token: `ol.${sessionId}` });
[ol.] git log -p --all, line 73061: +        '401 every time. Session ids (ol-<uuid>) and the owner token (ol. prefix, at least 24 characters, compared by hash in constant time) are separate names
[wallet address] git log -p --all, line 73609: +const WALLET_ADDRESS = '0xC75126992E4744a75665405e9b427710C0d23052';
[sk-ant] git log -p --all, line 73612: +  { name: 'sk-ant', test: /sk-ant/i },
[ol.] git log -p --all, line 73615: +  { name: 'ol.', test: /\bol\./ },
[PRIVATE_KEY] git log -p --all, line 73616: +  { name: 'PRIVATE_KEY', test: /PRIVATE_KEY/ },
[MNEMONIC] git log -p --all, line 73617: +  { name: 'MNEMONIC', test: /MNEMONIC/ },
[sk-ant] git log -p --all, line 73629: +const ANTHROPIC_KEY = /sk-ant-api\d{2}-[A-Za-z0-9_-]{40,}/;
[PRIVATE_KEY] git log -p --all, line 73631: +const KEY_ASSIGNMENT = /(PRIVATE_KEY|MNEMONIC)\s*[:=]\s*["']?[A-Za-z0-9+/=_-]{16,}/;
[MNEMONIC] git log -p --all, line 73631: +const KEY_ASSIGNMENT = /(PRIVATE_KEY|MNEMONIC)\s*[:=]\s*["']?[A-Za-z0-9+/=_-]{16,}/;
[ol.] git log -p --all, line 73643: +    test: (token) => token === 'ol.this-is-not-the-owners-token-at-all',
[ol.] git log -p --all, line 73788: +        'token that starts with ol. and is long enough to open the API and is not one of these,',
[ol.] git log -p --all, line 73840: +    'The other matches above are the word, not the thing: the "ol." prefix constant in the auth',
[ol.] git log -p --all, line 73993: +      token: 'ol.this-is-not-the-owners-token-at-all',
[ol.] git log -p --all, line 74258: +export const OWNER_TOKEN = 'ol.attack-run-token-not-for-production-1';
[ol.] git log -p --all, line 76309: +export const OWNER_TOKEN_PREFIX = 'ol.';
[ol.] git log -p --all, line 78101: +    .string({ error: 'not set, invent a secret that starts with "ol." and put it in .env' })
[ol.] git log -p --all, line 84199: +const TOKEN = `ol.${'k'.repeat(40)}`;
[ol.] git log -p --all, line 84555: +const TOKEN = `ol.${'e'.repeat(40)}`;
[ol.] git log -p --all, line 84814: +const TOKEN = `ol.${'b'.repeat(40)}`;
[sk-ant] git log -p --all, line 84853: +    ANTHROPIC_API_KEY: 'sk-ant-test-key-0123456789',
[ol.] git log -p --all, line 85611: +const OWNER_TOKEN = `ol.${'t'.repeat(40)}`;
[sk-ant] git log -p --all, line 85616: +      ANTHROPIC_API_KEY: 'sk-ant-test-key-0123456789',
[sk-ant] git log -p --all, line 85621: +      ANTHROPIC_API_KEY: 'sk-ant-test-key-0123456789',
[sk-ant] git log -p --all, line 85641: +      ANTHROPIC_API_KEY: 'sk-ant-test-key-0123456789',
[sk-ant] git log -p --all, line 85655: +      loadConfig({ ANTHROPIC_API_KEY: 'sk-ant-test-key-0123456789', OLAI_OWNER_TOKEN: token });
[ol.] git log -p --all, line 85658: +    expect(() => withToken('ol.short')).toThrowError(/24 characters/);
[wallet address] git log -p --all, line 89017: +            userWalletAddress: '0xC75126992E4744a75665405e9b427710C0d23052',
[wallet address] git log -p --all, line 89149: +  userWalletAddress: '0xC75126992E4744a75665405e9b427710C0d23052',
[ol.] git log -p --all, line 91528: +            placeholder="ol.…"
[ol.] git log -p --all, line 91554: +          Set OLAI_OWNER_TOKEN in .env, it starts with ol.
```
