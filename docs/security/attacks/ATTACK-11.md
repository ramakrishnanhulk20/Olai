...
Wrote a token through the same path the real sign-in uses:
  new BinanceOAuth({ tokenPath: "C:\Users\Ram\AppData\Local\Temp\olai-attack-11-CmIIWY\binance-mcp-token.json", ... }).authProvider().saveTokens({ access_token: "...", token_type: "Bearer" })
...
```
platform: win32
file: C:\Users\Ram\AppData\Local\Temp\olai-attack-11-CmIIWY\binance-mcp-token.json
